function keepUser(){
    firebase.auth().onAuthStateChange(user =>{ //usuario vai ser nulo se não tiver logado, se tiver logado vai ter valor
        if (user){
            window.location.href = "../home/homecat.html";
        }
    })
}
...
function getErrorMessage(error){
    if (error.code == "auth/invalid-credential"){
        return "Email ou senha incorretos. Verifique se está fazendo login no lado correto da aplicação. Você tentou realizar login como: CATADOR."
    }
